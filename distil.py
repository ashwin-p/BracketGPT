...
def create_train_state(model, rng, learning_rate, weight_decay, total_steps, input_shape):
...
def train():
    ...
    options = ocp.CheckpointManagerOptions(max_to_keep=1, create=True,
                                           step_prefix="tiny_bracket_lm_distilled")
    ...
    teacher_abstract_state = jax.tree_util.tree_map(ocp.utils.to_shape_dtype_struct,
                                                    teacher_state)
    ...
    restored = ckptr.restore(
        "/home/ashwin/Python/JAX/bracket_lm/bracketlm_checkpoints/bracket_lm_15",
        args=ocp.args.Composite(
            default=ocp.args.StandardRestore(teacher_abstract_state),
        ),
    )

    teacher_state = restored["default"]

    param_count = sum(x.size for x in jax.tree_util.tree_leaves(student_state.params))
    ...
    log_path = "logs/tiny_bracket_lm_distillation_metrics.csv"
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(pbar):
            student_state, metrics = train_step(student_state, batch, teacher_state)
